using line_bot_sdk_csharp.Core.Exceptions;
using line_bot_sdk_csharp.Webhook.Event;
using Newtonsoft.Json;
using System.Security.Cryptography;
using System.Text;
using Microsoft.Azure.Functions.Worker.Http;

namespace line_bot_sdk_csharp.Webhook
{
    public static class WebhookRequestMessageHelper
    {
        ...
        public static async Task<IEnumerable<WebhookEvent>> GetWebhookEventsAsync(this HttpRequestData request, string channelSecret, string? botUserId = null)
        {
            if (request is null) { throw new ArgumentNullException(nameof(request)); }
            if (channelSecret is null) { throw new ArgumentNullException(nameof(channelSecret)); }

            request.Body.Position = 0;
            string content = new StreamReader(request.Body, Encoding.UTF8).ReadToEnd();
